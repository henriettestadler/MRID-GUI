...
class Minimap:
    ...
    def __init__(self,load_mri):
        ...
        self.size_rectangle: dict[str, list[float]] = {view: [] for view in ("axial", "sagittal", "coronal")}
        self.limits_rectangle: dict[str, list[float]] = {view: [] for view in ("axial", "sagittal", "coronal")}
        self.zoom_rects = {}
        self.last_center_x_norm: dict[str, list[float]] = {view: [] for view in ("axial", "sagittal", "coronal")}
        self.last_center_y_norm: dict[str, list[float]] = {view: [] for view in ("axial", "sagittal", "coronal")}
        self.minimap_borders = {}
        self.half_width = {}
        self.half_height = {}


    def add_minimap(self,view_name:str,img_vtk:vtk.vtkImageData):
        """
        ...
        if view_name not in self.minimap_renderers:
            mm_renderer = vtk.vtkRenderer()
            rw, rh = self.load_mri.renderers[view_name].GetSize()
            w =  min(0.3,rh/rw*0.3)
            h =  min(0.3,rw/rh*0.3)
            self.size_rectangle[view_name] = [w,h]
        ...
        if hasattr(self, "minimap_borders") and view_name in self.minimap_borders:
            mm_renderer.RemoveActor(self.minimap_borders[view_name])
        ...
        self.limits_rectangle[view_name]= [display_min[0],display_max[0],display_min[1],display_max[1]]
        ...
        self.minimap_borders[view_name] = border_actor

        if not self.load_mri.zoom_tf[view_name]: # Hide at the beginning
            self.minimap_borders[view_name].SetVisibility(False)
            self.minimap_actors[view_name].SetVisibility(False)

    def create_small_rectangle(self,factor:float,vn: str=None,new_x: float=0,new_y: float=0,new_z: float=0):
        """
        ...
        for view_name, mm_renderer in self.minimap_renderers.items():
            camera = self.load_mri.renderers[view_name].GetActiveCamera()
            half_height[view_name] = camera.GetParallelScale()
            aspect = self.load_mri.renderers[view_name].GetSize()[0] / self.load_mri.renderers[view_name].GetSize()[1]
            half_width[view_name] = half_height[view_name] * aspect


            xmin, xmax, ymin, ymax = Zoom.bounds[view_name]
            xminR, xmaxR, yminR, ymaxR,_,_ = self.load_mri.renderers[view_name].ComputeVisiblePropBounds()
            tolerance = 1e-3

            if (xmin <= xminR + tolerance and xmax >= xmaxR - tolerance and ymin <= yminR + tolerance and ymax >= ymaxR - tolerance):
                self.load_mri.zoom_tf[view_name]=False
                self.minimap_borders[view_name].SetVisibility(False)
                self.minimap_actors[view_name].SetVisibility(False)
                if view_name in self.zoom_rects:
                    self.zoom_rects[view_name].SetVisibility(False)
                mm_renderer.GetRenderWindow().Render()
            else:
                self.load_mri.zoom_tf[view_name]=True
                self.minimap_borders[view_name].SetVisibility(True)
                self.minimap_actors[view_name].SetVisibility(True)
                if view_name in self.zoom_rects:
                    self.zoom_rects[view_name].SetVisibility(True)
        ...
        if not self.load_mri.zoom_tf['axial'] and not self.load_mri.zoom_tf['coronal'] and not self.load_mri.zoom_tf['sagittal']:
            for mm_renderer in self.minimap_renderers.values():
                mm_renderer.GetRenderWindow().Render()
            return

        if (new_x!=0 or new_y!=0):
            mm_renderer = self.minimap_renderers[vn]
            ...
            if vn == 'axial':
                ...
                mm_s = self.minimap_renderers['sagittal']
                ...
                mm_c = self.minimap_renderers['sagittal']
            ...
            if self.load_mri.zoom_tf['axial']: #axial
                ...
                points_a.InsertNextPoint(ax_min, ay_min, 0)
            if self.load_mri.zoom_tf['sagittal']:#sagittal
                ...
                points_s.InsertNextPoint(sx_min, sy_min, 0)
            if self.load_mri.zoom_tf['coronal']: #coronal
            ...
            self.pan_from_minimap('axial', [ax/window_width,ay/window_height])
            self.pan_from_minimap('sagittal', [sx/window_width,sy/window_height])
            self.pan_from_minimap('coronal', [cx/window_width,cy/window_height])
        else:
            for view_name, mm_renderer in self.minimap_renderers.items():
                window_width, window_height = mm_renderer.GetSize()
                ...
                if not self.load_mri.zoom_tf[view_name]:
                    continue
                points.InsertNextPoint(display_min[view_name][0], display_min[view_name][1], 0.2)
                points.InsertNextPoint(display_max[view_name][0], display_min[view_name][1], 0.2)
                points.InsertNextPoint(display_max[view_name][0], display_max[view_name][1], 0.2)
                points.InsertNextPoint(display_min[view_name][0], display_max[view_name][1], 0.2)
                points.InsertNextPoint(display_min[view_name][0], display_min[view_name][1], 0.2)

        for view_name, mm_renderer in self.minimap_renderers.items():
            if not self.load_mri.zoom_tf[view_name]:
                continue
            if view_name not in self.zoom_rects:
                poly_data = vtk.vtkPolyData()
                if new_x!=0 or new_y!=0:
                    if view_name == 'axial':
                ...
                mm_renderer.AddActor(actor)
                self.zoom_rects[view_name] = actor
                mm_renderer.GetRenderWindow().Render()
            else:
                # Update existing actor
                actor = self.zoom_rects[view_name]
    ...
    def pan_from_minimap(self, view_name, rect_pos):
        """
        Pan the main renderer camera based on the rectangle position in the minimap.
        """

        main_renderer = self.load_mri.renderers[view_name]
