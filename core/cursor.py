...
class Cursor:
    ...
    def init_widgets(self):
        ...
        lm = self.LoadMRI
        # spinboxes
        self.ui['spin_x'].setRange(1, lm.volume[0][0].shape[2])
        self.ui['spin_y'].setRange(1, lm.volume[0][0].shape[1])
        self.ui['spin_z'].setRange(1, lm.volume[0][0].shape[0])
        ...
        # scrollbars
        self.ui['scroll_x'].setRange(0, lm.volume[0][0].shape[2]-1)
        self.ui['scroll_y'].setRange(0, lm.volume[0][0].shape[1]-1)
        self.ui['scroll_z'].setRange(0, lm.volume[0][0].shape[0]-1)
    ...
    def update_cursor_display(self):
        ...
        for i in range(self.LoadMRI.num_data_max): #only for the first image
            intensity = lm.volume[i][0][lm.slice_indices[0],lm.slice_indices[1],lm.slice_indices[2]]
            self.LoadMRI.intensity[i] = intensity
            if self.ui[f"intensity{i}"] is not None:
                self.ui[f"intensity{i}"].setText(f"{intensity:.3f}")
    ...
    def set_line_points(self, view_name:str, line_h: vtk.vtkLineSource, line_v: vtk.vtkLineSource)-> tuple[vtk.vtkLineSource, vtk.vtkLineSource]:
        ...
        """
        lm = self.LoadMRI
        z = lm.slice_indices[0]*lm.spacing[0][0]
        y = lm.slice_indices[1]*lm.spacing[0][1]
        x = lm.slice_indices[2]*lm.spacing[0][2]
        height = 1
        if view_name == "axial":
            if self.LoadMRI.axes_to_flip[1]==False:
                line_h.SetPoint1(0, y, height)
                line_h.SetPoint2((lm.volume[0][0].shape[2]-1)*lm.spacing[0][2], y, height)
            else:
                line_h.SetPoint1(0, lm.volume[0][0].shape[1]*lm.spacing[0][1]-y, height)
                line_h.SetPoint2((lm.volume[0][0].shape[2]-1)*lm.spacing[0][2], lm.volume[0][0].shape[1]*lm.spacing[0][1]-y, height)
            if self.LoadMRI.axes_to_flip[0]==False:
                line_v.SetPoint1(x, 0, height)
                line_v.SetPoint2(x, (lm.volume[0][0].shape[1]-1)*lm.spacing[0][1], height)
            else:
                line_v.SetPoint1(lm.volume[0][0].shape[2]*lm.spacing[0][2]-x, 0, height)
                line_v.SetPoint2(lm.volume[0][0].shape[2]*lm.spacing[0][2]-x, (lm.volume[0][0].shape[1]-1)*lm.spacing[0][1], height)
        elif view_name == "coronal":
            if self.LoadMRI.axes_to_flip[2]==False:
                line_h.SetPoint1(0, z, height)
                line_h.SetPoint2((lm.volume[0][0].shape[2]-1)*lm.spacing[0][2], z, height)
            else:
                line_v.SetPoint1((lm.volume[0][0].shape[0]-1)*lm.spacing[0][0]-z, 0, height)
                line_v.SetPoint2((lm.volume[0][0].shape[0]-1)*lm.spacing[0][0]-z, (lm.volume[0][0].shape[1]-1)*lm.spacing[0][1], height)
            if self.LoadMRI.axes_to_flip[0]==False:
                line_v.SetPoint1(x, 0, height)
                line_v.SetPoint2(x, (lm.volume[0][0].shape[0]-1)*lm.spacing[0][0], height)
            else:
                line_v.SetPoint1(lm.volume[0][0].shape[2]*lm.spacing[0][2]-x, 0, height)
                line_v.SetPoint2(lm.volume[0][0].shape[2]*lm.spacing[0][2]-x, (lm.volume[0][0].shape[0]-1)*lm.spacing[0][0], height)
        elif view_name == "sagittal":
            if self.LoadMRI.axes_to_flip[1]==False:
                line_h.SetPoint1(0, y, height)
                line_h.SetPoint2((lm.volume[0][0].shape[0]-1)*lm.spacing[0][0], y, height)
            else:
                line_h.SetPoint1(0, lm.volume[0][0].shape[1]*lm.spacing[0][1]-y, height)
                line_h.SetPoint2((lm.volume[0][0].shape[0]-1)*lm.spacing[0][0], lm.volume[0][0].shape[1]*lm.spacing[0][1]-y, height)
            if self.LoadMRI.axes_to_flip[2]==False:
                line_v.SetPoint1((lm.volume[0][0].shape[0]-1)*lm.spacing[0][0]-z, 0, height)
                line_v.SetPoint2((lm.volume[0][0].shape[0]-1)*lm.spacing[0][0]-z, (lm.volume[0][0].shape[1]-1)*lm.spacing[0][1], height)
            else:
                line_v.SetPoint1(z, 0, height)
                line_v.SetPoint2(z, (lm.volume[0][0].shape[1]-1)*lm.spacing[0][1], height)
    ...
    def update_cursor_from_interactor(self, interactor, view_name:str):
        ...
        if suc:
            ...
            if view_name == "axial":
                if lm.axes_to_flip[0]==False:
                    xi = pos[0]/lm.spacing[0][2]
                else:
                    xi = lm.volume[0][0].shape[2]-1-pos[0]/lm.spacing[0][2]
                if lm.axes_to_flip[1]==False:
                    yi = pos[1]/lm.spacing[0][1]
                else:
                    yi = lm.volume[0][0].shape[1]-1-pos[1]/lm.spacing[0][1]
                zi = old_indices[0]
            elif view_name == "sagittal":
                xi = old_indices[2]
                if lm.axes_to_flip[1]==False:
                    ...
                    yi = lm.volume[0][0].shape[1]-1-pos[1]/lm.spacing[0][1]
                if lm.axes_to_flip[2]==False:
                    zi = lm.volume[0][0].shape[0]-1-pos[0]/lm.spacing[0][0]
                else:
                    zi = pos[0]/lm.spacing[0][0]
            elif view_name == "coronal":
                if lm.axes_to_flip[0]==False:
                    xi = pos[0]/lm.spacing[0][2]
                else:
                ...
                yi = old_indices[1]
                if lm.axes_to_flip[2]==False:
                    zi = pos[1]/lm.spacing[0][0]
                else:
                    zi = lm.volume[0][0].shape[0]-1-pos[1]/lm.spacing[0][0]

            zi = max(0, min(zi, lm.volume[0][0].shape[0]-1))
            yi = max(0, min(yi, lm.volume[0][0].shape[1]-1))
            xi = max(0, min(xi, lm.volume[0][0].shape[2]-1))
    ...
    def scroll_slice(self, view_name:str, delta:int,val:int=-1):
        ...
        if val != -1:
            if view_name == 'z':
                lm.slice_indices[0] = np.clip(val, 0, lm.volume[0][0].shape[0]-1)
            elif view_name == 'y':
                lm.slice_indices[1] = np.clip(val, 0, lm.volume[0][0].shape[1]-1)
            elif view_name == 'x':
                lm.slice_indices[2] = np.clip(val, 0, lm.volume[0][0].shape[2]-1)
        else:
            if view_name == 'axial':
                lm.slice_indices[0] = np.clip(lm.slice_indices[0] + delta, 0, lm.volume[0][0].shape[0]-1)
            elif view_name == 'coronal':
                lm.slice_indices[1] = np.clip(lm.slice_indices[1] + delta, 0, lm.volume[0][0].shape[1]-1)
            elif view_name == 'sagittal':
                lm.slice_indices[2] = np.clip(lm.slice_indices[2] + delta, 0, lm.volume[0][0].shape[2]-1)
